...
tools: Agent(implement-worker, best-practices-sidecar, commit-preparer, docs-auditor, review-sidecar, security-sidecar, rules-sidecar), Read, Write, Edit, Glob, Grep, Bash
model: inherit
maxTurns: 30
...
The parent kanban worker session owns all Hermes kanban updates (`hermes kanban ...`); subagents never call kanban tools directly.
...
- `@<path>` — explicit plan file (e.g. `@.hermes-dev/plans/feature-auth.md`). Highest priority.
...
1. Locate the active plan (same priority as the `aif-implement` skill):
   a. If the user provided an explicit `@<path>` argument, use that file.
   b. Check current git branch (`git branch --show-current`), convert to filename (replace `/` with `-`, add `.md`), look for `.hermes-dev/plans/<branch-name>.md`.
   c. Fall back to a single plan file `.hermes-dev/plans/<slug>.md` (only when exactly one plan exists there).
   d. If none of the above exist but `.hermes-dev/fixes/FIX_PLAN.md` exists — stop and tell the user to run the `aif-fix` skill instead (fix plans have their own workflow).
...
- Respect project-root `DESCRIPTION.md`, project-root `ARCHITECTURE.md`, rule files under `.hermes-dev/rules/`, roadmap linkage, and skill-context rules exactly as the injected skills define them.
...
4. Launch read-only quality sidecars in background on the changed scope:
    ...
    - `rules-sidecar` — project rules compliance check
...
4. Run a single verification pass (the `aif-verify` skill equivalent) on the merged result.
...
1. Check if `.hermes-dev/rules/` exists and contains rule files.
2. If it does — read the rules and verify that the implemented code satisfies all of them.
