...
description: >-
  ...
  Covers naming, structure, error handling, testing, and code review standards.
  Use when writing code, reviewing, refactoring, or asking "how should I name this",
  "best practice for", "clean code". Advisory knowledge skill for the AI Factory
  department (port of lee-to AI Factory /aif-best-practices, adapted to Hermes).
tags:
  - aif
  - best-practices
  - code-quality
...
- Advisory, read-only reference skill for the AI Factory department (pipeline & conventions: `aif-methodology`).
  Loaded by `aif_implementer` / `aif_reviewer` workers and usable interactively.
- **Project context:** if `ARCHITECTURE.md` exists (project root; some apps keep it under `.hermes-dev/`),
  follow its folder structure, dependency rules, and module boundaries alongside these guidelines.
  Hard project rules in `.hermes-dev/RULES.md` (+ `.hermes-dev/rules/*`) override general patterns here.
- **Read `.hermes-dev/skill-context/aif-best-practices/SKILL.md` — MANDATORY if it exists.**
  It contains project-specific rules accumulated by `aif-evolve`. Treat them as project-level
  overrides: on conflict with this file, **skill-context wins** (more specific context takes priority).
  When there is no conflict, apply both. Skill-context rules apply to ALL outputs of this skill —
  recommendations, examples, and checklists included. After generating any output, verify it against
  all skill-context rules and fix violations before presenting.

## Sidecar delegation

Where a coordinating skill (e.g. `aif-implement` in coordinator flows) wants a read-only
maintainability audit of the current change against these guidelines: **if subagents mode is
enabled** (department default), spawn the `best-practices-sidecar` subagent via the Agent tool —
it returns a structured verdict (`Verdict: PASS|WARN|FAIL`, `Blocking findings:`,
`Non-blocking notes:`, `Evidence:`). **In skills mode**, apply this skill inline yourself
(read-only: Read/Glob/Grep) and produce the same verdict shape.

## Quick Reference

Load skill `aif-best-practices`, optionally scoped by topic:
...
- Follow repository context, `ARCHITECTURE.md`, `.hermes-dev/RULES.md`, and skill-context
  overrides. There is no `config.yaml` in Hermes — paths and conventions are fixed by `aif-methodology`.
