...
name: aif-ci
description: >-
  Analyze a project and generate a production-grade CI/CD pipeline (GitHub Actions or
  GitLab CI) with separate jobs for linting, static analysis, tests, and security scanning —
  or audit/enhance an existing pipeline. Use when a dev task says "ci", "setup ci",
  "github actions", "gitlab ci", "pipeline", "add ci checks". Port of lee-to AI Factory
  /aif-ci, adapted to Hermes.
tags:
  - aif
  - ci
  - pipelines
...
# aif-ci — Pipeline Configuration Generator
...
|-------------|------|--------|
| No CI config | `generate` | Create pipeline from scratch |
...
- Skill artifacts (this file, `references/`, `templates/`) live in `~/.hermes/skills/aif-ci/`; project artifacts under the app workspace (`~/Work/apps/<slug>/`).
- **Autonomous:** no interactive questions in kanban mode. Proceed on evidence (repo detection); if genuinely required input is missing or an action would affect production, **block the kanban task with the exact reason**. Interactive (topic/CLI) sessions may ask the user instead.
- **Safety (block-not-ask):** CI configuration is *written to the project* — that is always in scope. But actions that trigger real pipelines or deployments on production infrastructure (pushing workflow files to a live default branch that auto-deploys, enabling scheduled prod jobs, registering runners, setting live secrets) require explicit human approval → `kanban_block` with the exact pending action. Committing to a work branch is fine when committing is in scope for the workspace.
...
**Read `.hermes-dev/skill-context/aif-ci/SKILL.md`** — MANDATORY if the file exists.
...
Also read the kanban task body + parent handoffs (`hermes kanban --board departments show <id>`): they may name the platform (`github`/`gitlab`), request `--enhance`, or scope specific jobs.
...
**If the task body contains `--enhance`** → set `MODE = "enhance"` regardless.

**Path A: No CI config exists** (`!HAS_GITHUB_ACTIONS && !HAS_GITLAB_CI && !HAS_OTHER_CI`):
- Set `MODE = "generate"`
- Proceed to **Step 1.3: Platform & Feature Selection**
...
### 1.3 Platform & Feature Selection (Generate Mode Only)

**Determine CI platform on evidence, in this order:**

1. Task body names it (`github` / `gitlab`) → use that.
2. `git remote -v`: remote on `github.com` → `PLATFORM = "github"`; on `gitlab.com`/self-hosted GitLab → `PLATFORM = "gitlab"`.
3. No remote / ambiguous → default `PLATFORM = "github"` (GitHub Actions) and state the assumption in the handoff. In an interactive session you may ask the user instead.

**Determine optional features on evidence** (do not ask in kanban mode; interactive sessions may confirm):

- `WANT_SECURITY`: true if security tools are installed/configured in the project (see Step 2, security audit tools) or the task requests security scanning.
- `WANT_COVERAGE`: true if coverage tooling is configured (coverage config, coverage scripts, a coverage service) or the task requests it.
- `WANT_MATRIX`: true only if the task requests it or the project's version constraint clearly spans multiple supported versions AND multi-version support is a stated goal. Default false — no matrix for single-version projects.
...
```
Read references/BEST-PRACTICES.md      # in this skill's folder (~/.hermes/skills/aif-ci/)
```
...
```
Read templates/<platform>/<language>.yml
```
...
For audit report format, fix flow, and display templates → read `references/AUDIT-REPORT.md`

Present results as tables with ✅/❌/⚠️ per check. Categorize recommendations by severity (CRITICAL, HIGH, MEDIUM, LOW).

**Fix flow (autonomous):** in kanban mode apply all CRITICAL and HIGH recommendations, plus MEDIUM/LOW ones that are safe and in scope; include the full audit table and what was fixed vs deferred in the handoff. In an interactive session you may ask the user to choose (fix all / critical only / show details first).
...
Build the summary using the format from `references/AUDIT-REPORT.md` (Summary Display Template section): platform, files created, features, quick start commands.

### 7.2 Complete the kanban task

Return a structured handoff (files written, mode, platform, features, audit findings fixed/deferred, assumptions made) and **complete on evidence** via `kanban_complete`. Do not self-block waiting for human approval — only block if a real prod-affecting action is pending (see Hermes context above).

Suggest follow-ups in the handoff: load skill `aif-build-automation` for CI targets in Makefile/Taskfile, load skill `aif-dockerize` for containerization.
...
- Config policy: config-agnostic by design. This skill relies on repository detection and explicit task-body choices — Hermes has no `config.yaml`; paths are fixed.
