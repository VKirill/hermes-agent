...
name: aif-qa-check
description: >-
  Execute the QA test cases produced by aif-qa, in human-guided or automated-agent mode,
  and record pass/fail results with evidence and staleness binding. Validates aif-qa
  output; emits the department gate_result (gate "qa-check"). Use when the user says
  "run the QA", "execute test cases", "check the QA plan", or as the kanban QA-execution
  gate after aif-qa. Port of lee-to AI Factory /aif-qa-check, adapted to Hermes.
tags:
  - aif
  - qa
  - testing
...
# aif-qa-check — Execute Test Cases (QA execution gate)
...
- Runs as a kanban worker (QA execution gate) or interactively. **No `HANDOFF_MODE`, no `.ai-factory/config.yaml`** — paths are fixed:
  - `qa_root = .hermes-dev/qa/`
  - `qa_agent_context_path = <qa_root>/agent-context.md`
  - `qa_agent_history_path = <qa_root>/agent-history.md`
  - Project context: `DESCRIPTION.md`, `ARCHITECTURE.md` (project root); project rules: `.hermes-dev/skill-context/aif-qa-check/SKILL.md`
- **Kanban workers always run agent mode.** Human mode is interactive-only (topic/CLI) — it needs a person at the keyboard.
- **Autonomous:** workers do not ask questions. Where this skill says "ask the user", a worker first exhausts `agent-context.md`, `agent-history.md`, fixtures/seeders, and project docs; if the input is still missing, the case is set `Blocked` with the exact missing input, and the run continues with other cases. Production/unknown targets and destructive actions are NEVER executed by a worker — those cases are always `Blocked` pending explicit human authorization. Interactive sessions ask as written.
- **Gate skill:** emits the department gate_result contract (gate `"qa-check"`) — see "Gate result" below and `aif-methodology` for the full contract.
...
If no mode is provided: worker → `agent`; interactive → ask which mode to run.
...
Fixed settings (no config file in Hermes): `qa_root`, `qa_agent_context_path`, `qa_agent_history_path` as above; `git_enabled = true` unless the workspace is not a git work tree or the task body says otherwise.

**Artifact language:** `qa-check.md` is written in English by default. If the kanban task, topic conversation, or project context explicitly requests another language (e.g. Russian), translate headings, labels, status text, agent-authored comments, and explanatory text before saving — while ALWAYS preserving checkbox syntax, test case IDs (`TC-001`), commands, paths, config keys, URLs, selectors, package names, API names, branch names, and raw error messages, and preserving user wording in recorded comments except mandatory redaction of sensitive values. The canonical English template is `templates/QA-CHECK.md`; it defines structure, not language. User-visible messages follow the language of the conversation.

### Step 0.1: Load Project Context

Read `DESCRIPTION.md` and `ARCHITECTURE.md` (project root) if they exist.

Read `.hermes-dev/skill-context/aif-qa-check/SKILL.md` — MANDATORY if the file exists. Treat it as project-level overrides for this skill.

Read `<qa_root>/agent-context.md` if it exists. Treat it as reusable, user-approved QA execution context for future automated runs: stable environment classes, canonical local/staging URLs, login route, test account role, allowed non-sensitive usernames, required seed data patterns, stable selectors, field identifiers, known redirects, startup prerequisites, safe command patterns, reusable test-filter conventions, environment notes, and service startup instructions.

Read `<qa_root>/agent-history.md` if it exists. Treat it as append-only cross-QA memory from prior runs. Before asking the user or attempting automated execution, search it for reusable prior answers to the same blocker so the skill does not repeat avoidable mistakes.

`agent-context.md` and `agent-history.md` are global to all QA sets under `qa_root`. They MUST NOT become logs for a specific QA plan. Do not write branch names, branch slugs, QA target paths, current `artifact_dir`, `TC-*` mappings, per-run summary counts, exhaustive command transcripts, assertion counts from a single run, one-off file checks, or plan-specific labels such as a numbered QA folder name. Write those details only to `<qa_root>/<branch-slug>/qa-check.md`.
...
Never write global negative capability or scope claims unless they are true for the whole project. Do not write statements like `browser automation is not required`, `browser URL is not used`, `all QA is backend/CLI`, or `these cases are backend-test/cli/file-docs` to the shared memory files when that conclusion came from one QA plan. Instead, keep it branch-specific in `qa-check.md`, or phrase a reusable conditional rule such as `For backend-only QA cases, browser automation is not required; browser/UI cases still require a live browser capability`.
...
Parse the invocation arguments (or kanban task body):
...
3. If no mode was provided: worker → `agent`; interactive → ask (human-guided QA / automated-agent QA).
...
Store:
- `resolved_branch`
- `artifact_dir = <qa_root>/<branch-slug>`
...
If it is missing, STOP: worker → `kanban_block` with "test-cases.md missing — run aif-qa test-cases <resolved_branch> first"; interactive → ask whether to run `aif-qa test-cases <resolved_branch>` first, or cancel.
...
- `manual_build_id` — when `git_enabled = false` or the repository is not a git work tree, obtain an explicit build/version identifier before creating or resuming results (worker: from the task body, else block; interactive: ask). Do not accept an empty identifier.
...
- If `worktree_digest` changed, mark every prior result as `Stale` and unchecked, preserve prior comments/evidence as historical context, and require retest.
...
- If a prior case no longer exists in `test-cases.md`, keep its historical entry marked `Stale` or move it to a "Stale / Removed Cases" section; never count it as current.
...
Agent mode can perform live browser actions, shell commands, local service calls, and other checks with meaningful side effects — run this preflight before executing any case or writing `qa-check.md`:
...
1. Detect available execution capabilities from the current runtime tools:
   ...
   - live browser automation (in-app Browser capability or Playwright MCP)
...
4. If a case requires browser automation, prefer the in-app Browser capability if available; otherwise use Playwright MCP if available.
5. If a browser/UI case requires live browser automation and neither is available, keep only that case unchecked, set status to `Blocked` (worker: record "live browser capability unavailable"; interactive: also ask the user to enable one), append this blocker to `<qa_root>/agent-history.md`, and continue with other cases that can be verified through CLI, tests, API, or file/document checks.
6. Use `agent-context.md`, `agent-history.md`, `test-cases.md`, `change-summary.md`, project docs, current browser state, and repository scripts to determine the target URL, service startup commands, test commands, and environment.
7. For browser/UI cases that require authentication or specific user state, resolve a browser test identity before marking the case blocked:
   ...
   - If creating a fixture requires a password, role, tenant, workspace, feature flag, seed command, or any unclear setup decision — interactive: ask before blocking (existing test account vs. authorize a disposable fixture); worker: block the case with the exact decision needed.
...
8. If a user-suppliable prerequisite is missing (application/login URL; test account role or non-sensitive username; where to find credentials without exposing them; authorization to create a disposable local/test fixture and desired initial state; required startup state, seed data, feature flag, tenant, organization, or workspace; ambiguous login mechanism or auth provider; stable selector, field identifier, or page route; exact project command, test filter, service startup command, or safe API endpoint) — interactive: ask before blocking; worker: block the case naming exactly which prerequisite is missing.
9. Save reusable non-sensitive answers to `<qa_root>/agent-context.md` immediately only when the answer is general enough to help future QA sets. Append the redacted answer summary to `<qa_root>/agent-history.md` only when it captures a reusable lesson; otherwise keep it in the branch-specific `qa-check.md` evidence/comment.
...
11. Do not execute browser actions, API writes, CLI commands with side effects, database writes, destructive commands, payment, permission, email, notification, data export, or other external-side-effect steps against `production` or `unknown` targets without explicit user authorization immediately before execution. Kanban workers can never obtain this authorization mid-run — they MUST block such cases. Record the authorization decision and target class in `qa-check.md`. Append it to `agent-history.md` only when it creates a reusable cross-QA policy; do not persist broad production authorization for future runs.
...
14. If authorization is denied, unclear, or unavailable (worker), leave the case unchecked, set status to `Blocked`, and write the blocker to `qa-check.md` without executing the risky action. Append the decision to `agent-history.md` only when it is a reusable cross-QA lesson, not merely a case-specific denial.
...
### Step 2: Human Mode (interactive only)
...
2. Include a short per-case summary in the conversation language.
3. The summary question MUST mean: "Test this and answer whether it works." For Russian conversations, use exactly: `Протестируйте и ответьте работает или нет`.
...
- If no URL, command, startup state, credentials location, test filter, or required fixture is clear — interactive: ask the user, then persist only the non-sensitive reusable facts to `agent-context.md`; worker: block the affected case with the exact missing detail.
...
1. Select the execution surface from the case classification:
   - `browser-ui`: navigate with the live browser capability and execute UI steps.
   ...
   - `database-read`: use only safe read-only checks unless mutation in a test-scoped environment is explicitly authorized.
...
3. If execution stalls because of missing credentials, missing browser fixture, unavailable services, unclear setup, blocked navigation, missing selector knowledge, ambiguous field mapping, unknown route, unknown command, missing fixture, or unclear test filter — search `agent-context.md` and `agent-history.md` first. For browser/UI auth or user-state gaps, try safe local/test fixture discovery or creation as described in Step 1.1 before blocking. If the answer or fixture is still not available — interactive: ask the user exactly for the missing information or authorization before marking the case blocked; worker: block the case with the exact missing input.
4. When usable non-sensitive information becomes available, continue the case in the same run. Update `agent-context.md` with durable cross-QA facts and append the resolved friction to `agent-history.md` only if it is likely to recur in future QA sets. Keep case-specific details in `qa-check.md`.
5. If the missing information cannot be obtained, the service remains unavailable, authorization is denied, the required capability is unavailable, or a side-effect risk remains unresolved, keep the checkbox unchecked (`[ ]`), set status to `Blocked`, and write the blocker.
...
- Update `<qa_root>/agent-context.md` when a discovered fact is durable and likely useful for future QA sets.
- Append to `<qa_root>/agent-history.md` only when a blocker, selector, command pattern, test-filter convention, API endpoint pattern, route, setup note, or authorization decision is likely to recur beyond the current QA set.
...
Keep `<qa_root>/agent-context.md` concise and current; prefer stable facts over one-off observations. Do not write raw passwords, tokens, cookies, authorization headers, one-time codes, private personal data, token-bearing URLs, branch names, QA target paths, or run summaries.
...
### Step 4: Final Summary and Gate Result

After stopping or finishing, report:
- total test cases; passed; failed; blocked; pending
- path to `qa-check.md`
...
- Human-verifiable = execution surface `human`, `browser-ui`, `hybrid` with human-observable steps, or `unknown`.
...
- **Interactive:** if at least one human-verifiable blocked case exists, ask whether to run those eligible cases now in human-guided mode. If yes, continue in the same invocation using Step 2, only for eligible blocked cases; preserve existing agent blocker comments as context and append the human result/comment under the same case. If declined, keep them unchecked and `Blocked`.
- **Worker:** list human-verifiable blocked cases in the handoff and recommend an interactive `aif-qa-check human <resolved_branch>` run for them.
...
If any case failed, the next recommended action is to fix the issue and rerun `aif-qa-check <mode> <resolved_branch>`.

If human-verifiable cases remain blocked after an agent-mode run, the next recommended action is `aif-qa-check human <resolved_branch>` for those cases or providing the missing execution context. If automation-only cases remain blocked, recommend adding/running the required automated test, command, fixture, service, or context instead. Do not imply that a blocked case is a product defect unless there is concrete observed evidence.

**Gate result:** finish the kanban run by emitting **exactly one** machine-readable gate_result block (schema in `aif-methodology` — do not restate it) and pass it via `kanban_complete` (pass/warn) or `kanban_block` (fail):
- `gate`: `"qa-check"`
- `status: "pass"` — every current case `Passed`
- `status: "warn"` — no `Failed` cases, but some cases `Blocked`/`Pending` for recorded automation-prerequisite or authorization reasons (list them in `blockers` with `blocking: false` context)
- `status: "fail"` — any current case `Failed`, or QA could not run at all; `blockers` list the failed case IDs with observed-vs-expected one-liners
- `affected_files`: `qa-check.md` path + files/commands central to failures; `suggested_next`: `/aif-fix` on fail, `null` or the human-run recommendation otherwise
- Per the department no-stall laws (`aif-methodology`): a fail verdict MUST result in a fix task for `aif_implementer` (idempotency-key, linked for re-check) — never leave a failed case without a fixer.

## Artifact Ownership

- Primary ownership: `<qa_root>/<branch-slug>/qa-check.md`, `<qa_root>/agent-context.md`, and `<qa_root>/agent-history.md`.
- Read policy: may read `<qa_root>/<branch-slug>/change-summary.md`, `test-plan.md`, `test-cases.md`, `agent-context.md`, and `agent-history.md` as QA context.
- Write policy: persistent writes are limited to `qa-check.md`, `agent-context.md`, and `agent-history.md`; do not rewrite `test-cases.md`, `test-plan.md`, or `change-summary.md`.
...
10. MUST require explicit authorization for production/unknown targets and destructive or external-side-effect cases; kanban workers MUST block such cases instead of executing them.
...
13. MUST read `agent-context.md` and `agent-history.md` before automated execution and before asking for setup facts, so prior answers are reused.
14. MUST exhaust recoverable sources for missing execution context (URL, credentials location, login route, field identifiers, setup state, selectors, commands, test filters, fixtures) — interactive: ask the user; worker: search memory/fixtures/docs — before blocking an agent-mode case.
15. MUST NOT block a browser/UI case for missing login credentials or user state until it has searched reusable context, existing fixtures, seeders/factories, docs, and safe local/test data; attempted safe disposable fixture creation when the environment is local/test and tooling is available; or (interactive) asked the user for credentials, fixture setup, or authorization to create a disposable fixture.
...
18. Interactive agent runs MUST ask whether to continue eligible blocked cases in human mode before ending with any current human-verifiable `Blocked` cases; worker runs MUST surface those cases in the handoff/gate_result instead.
