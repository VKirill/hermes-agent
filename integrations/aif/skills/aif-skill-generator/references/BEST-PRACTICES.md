...
(For agents that honor `allowed-tools` — Hermes ignores this field, but keep it correct for
cross-agent skills.)
...
### Manual Testing
1. Load the skill by name (Hermes: mention it or let the model pick it up; Claude Code: `/skill-name`)
2. Invoke with arguments: `skill-name arg1 arg2`
3. Let the model invoke it naturally from a matching request
4. Test edge cases
...
# Integration test
# Ask the agent to use the skill in various contexts
```
