...
Hermes routing: as a kanban worker, pass this block via `kanban_complete` for `pass`/`warn` and `kanban_block` for `fail` (see `aif-methodology` for the shared gate contract).
