[← Development Workflow](workflow.md) · [AIF methodology](../../SKILL.md) · [Subagents →](subagents.md)

> Ported from lee-to AI Factory `docs/loop.md`. Path adaptation: loop state lives at the fixed
> `.hermes-dev/loop/` (lee-to: `paths.evolution`, default `.ai-factory/evolution/`).
> Delegation uses the Agent tool, gated by subagents mode.
...
Paths below are fixed — the loop-state root is always `.hermes-dev/loop/` in the target workspace.
...
Before iteration 1, `aif-loop new` must pin down:

1. Success criteria (rules + thresholds)
2. Max iterations (`run.json.max_iterations`)

In **interactive** use this is an explicit user confirmation, even when the task prompt already
contains criteria and an iteration count. In **kanban worker** mode (non-interactive) the loop
derives both from the task text or template defaults and records the chosen values in the loop
history before iteration 1 — it never silently invents stricter or looser criteria mid-run.

## Persistence Model

4 files total for loop persistence (1 global pointer + 3 per-loop files). `current.json` exists
only while a loop is active:
...
Single source of truth for artifact content. Written after PRODUCE and REFINE phases. Artifact
content is never stored in `run.json` — always read from this file.

Ownership note: `artifact.md` is owned by `aif-loop` for the active run. Other workflow skills
should treat loop artifacts as read-only context unless the user explicitly asks for manual edits.
...
Two levels of parallelism via the Agent tool (subagents mode — department default):

- **PRODUCE || PREPARE**: both depend only on PLAN output, run as parallel subagents
  (`loop-producer` || `loop-test-prep`/`loop-perf-prep`/`loop-invariant-prep`)
- **Within EVALUATE**: independent check groups (executable via Bash, content via Read/Grep) run
  as parallel subagents

In skills mode (delegation disabled), all phases execute sequentially inline.

### Phase Contracts

Strict I/O contracts are defined in skill references:

- `~/.hermes/skills/aif-loop/references/PHASE-CONTRACTS.md` — input/output/constraints per phase

## Evaluation Rules

Rules define what the evaluator checks. Runtime rules in `run.json.criteria.rules` always include
full schema fields (`id`, `description`, `severity`, `weight`, `phase`, `check`).
...
Severity levels: `fail` (weight 2, blocks pass), `warn` (weight 1, reduces score), `info`
(weight 0, tracked only).

Template rows are shorthand; during setup they are normalized to full runtime rules. If `weight`
is omitted, it is derived from severity (`fail`=2, `warn`=1, `info`=0). If task-specific checks are
needed, `check` is materialized before iteration starts.

Full schema and ID conventions: `~/.hermes/skills/aif-loop/references/RULE-SCHEMA.md`

### Criteria Templates

Pre-built rule sets for common task types (API spec, code generation, documentation,
configuration): `~/.hermes/skills/aif-loop/references/CRITERIA-TEMPLATES.md`
...
5. If phase A passed: switch to phase B, re-run `PREPARE` (phase=B) + `EVALUATE` against same
   artifact with B-level rules (no re-produce)
...
If stop reason is `iteration_limit` and latest evaluation is `passed=false`, summary must also
include **distance to success**:
...
After each iteration, show a **compact summary** — do not dump full `run.json` or `artifact.md`
into the conversation. The artifact is on disk; duplicating it wastes context.
...
2. **Phase A → B transition** — show the phase-A-passing artifact in full once at the transition
   boundary for visibility (B-level evaluation still runs immediately per iteration flow)
3. **Explicit user request** — user asks to see the full artifact

## Context Management

All loop state is persisted to disk. Clearing conversation context loses nothing — `resume`
reconstructs from files.

In interactive Claude Code sessions, `/clear` then `aif-loop resume` is recommended:

- After iteration 2 (midpoint of default 4-iteration loop)
- On Phase A → B transition
- When iteration >= 3

(Kanban workers get a fresh context per dispatch anyway — `resume` is the normal entry there.)
...
- **Missing `history.jsonl`**: inform the user / block the task, suggest starting a new loop
...
1. Keep architecture simple — phases run in a single agent context, parallelism only where inputs
   are independent (PRODUCE||PREPARE, check groups in EVALUATE).
...
- [Subagents](subagents.md) — the loop roles used to split planning, generation, and evaluation
- [Core Skills](skills.md) — full skill reference including `aif-loop`
- [Configuration](configuration.md) — `.hermes-dev/` storage layout
