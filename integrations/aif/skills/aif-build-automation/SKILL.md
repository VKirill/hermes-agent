...
description: >-
  Analyze a project and generate or enhance its build automation file (Makefile,
  Taskfile.yml, Justfile, Magefile.go). If a build file already exists, improve it by
  adding missing targets and best practices. Use when the user says "generate makefile",
  "create taskfile", "add justfile", "setup mage", "build automation", or a kanban task
  asks for build tooling. Port of lee-to AI Factory /aif-build-automation, adapted to Hermes.
tags:
  - aif
  - build-automation
  - tooling
...
- Runs in an app workspace (`~/Work/apps/<slug>/`; department conventions: `aif-methodology`).
  Skill assets live at `~/.hermes/skills/aif-build-automation/` (references/ + templates/).
- **Autonomy:** kanban workers do not ask interactive questions. Wherever this skill's original asked
  the user to choose, decide on evidence (defaults given inline below) and record the decision in the
  handoff. Interactive (topic/CLI) sessions MAY ask. Block only for real reasons (no workspace, empty repo).
- The requested tool comes from the skill argument or the kanban task body: `makefile | taskfile | justfile | mage`.
- On completion as a worker: `kanban_complete` with evidence — output path, mode (generate/enhance),
  targets table, `PROJECT_PROFILE` summary.

---

## Step 0: Load Project Context

Read the project description if available: `DESCRIPTION.md` (project root; some apps keep it under
`.hermes-dev/`). Store the project context (tech stack, framework, architecture) for use in later
steps. If the file doesn't exist, that's fine — we'll detect everything in Step 2.

**Read `.hermes-dev/skill-context/aif-build-automation/SKILL.md` — MANDATORY if it exists.**
...
the generated build files (Makefile, Taskfile, justfile, magefile); templates in this skill are **base
structures**. If a skill-context rule says "build file MUST include target X" or "MUST follow convention Y" —
comply. After generating any output artifact, verify it against all skill-context rules and fix violations
before finishing.
...
- If multiple build files exist AND the argument specifies one, use the argument to pick which one to enhance
- If multiple build files exist AND no argument: interactive — ask which one; worker — enhance the one
  the repo actually uses (referenced by CI config or README; if unclear, prefer Makefile → Taskfile →
  Justfile → Magefile) and note the choice in the handoff.
...
- Set `MODE = "generate"`
- Parse the argument to determine tool:
...
- If the argument is empty or doesn't match: interactive — ask (Makefile: universal, no install;
  Taskfile: YAML, modern, cross-platform; justfile: simple, ergonomic; magefile: Go-native, type-safe);
  worker — default to **Makefile** (universal, no install needed) and note the default in the handoff.
...
| Gradle **and** `pom.xml` | `gradle` | Set `java_build.mixed_maven_gradle: true` and append a **warning** to `PROJECT_PROFILE.warnings` (both builds present; recipes follow Gradle — confirm the authoritative build in the handoff). |
...
```
Read ~/.hermes/skills/aif-build-automation/references/BEST-PRACTICES.md
```
...
For PHP, Rust, Ruby, or Java/JVM + Magefile: Mage is Go-specific and not generally applicable to these stacks. If the request explicitly asked for `mage` on such a project: interactive — explain and ask whether to proceed with Makefile as the closest alternative (universal, no install needed); worker — proceed with Makefile and record the substitution + reason in the handoff.

Read the selected template:

```
Read ~/.hermes/skills/aif-build-automation/templates/<selected-template>
```
...
Display summary using format from `references/SUMMARY-FORMAT.md`. Shows targets table, project profile used, and quick start command for Mode B (generate), or what changed + new/existing targets for Mode A (enhance). Include installation hints if the tool requires setup. As a kanban worker, include this summary in the `kanban_complete` handoff.
...
Brief: scan for existing command sections, update or append quick reference, suggest AGENTS.md creation if missing (worker: do not create an AGENTS.md unrequested — note the suggestion in the handoff instead).
...
- There is no `config.yaml` in Hermes — this skill is driven by repository detection, `DESCRIPTION.md`, and `.hermes-dev/skill-context/` overrides only.
