...
**If NO agent instruction file exists:**

- **Interactive session** — suggest creating one: "This project doesn't have an AGENTS.md (AI agent
  instructions). Should I create one with build commands?" (Create / Skip).
- **Kanban worker** — do NOT create an unrequested AGENTS.md; note the suggestion in the completion
  handoff instead.

If creating it (user agreed, or the task explicitly asks), generate a minimal `AGENTS.md` with:
- Project name and brief description (from `PROJECT_PROFILE` or `DESCRIPTION.md`)
