...
```bash
python3 ~/.hermes/skills/aif-distillation/scripts/material-prep.py <source...>
```

Use `python`, `py -3`, or `py` only if that was the selected Python 3 command. If no probe reports Python 3, do not call the helper. Use direct reads/searches for text sources, ask the user for text/markdown exports for PDFs or very large sources (as a kanban worker: block with the exact ask), and state the coverage limitation in the final result.
...
```bash
python3 ~/.hermes/skills/aif-distillation/scripts/material-prep.py --cleanup <temp-dir>
```
...
2. Ask the user for a text/markdown export (kanban worker: block with the exact ask).
