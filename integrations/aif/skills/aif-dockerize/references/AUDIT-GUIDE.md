...
**Kanban (autonomous) mode:** do not ask. Apply all CRITICAL and HIGH recommendations, plus MEDIUM/LOW ones that are safe and within the task scope. Record in the handoff which recommendations were applied and which were deferred (with a one-line reason each). Optionally export the full report to `.hermes-dev/docker-audit.md` when the task asks for a report artifact.
...
4. Export report — Save audit report to `.hermes-dev/docker-audit.md`

**If fixing:**
- For Dockerfile issues → edit existing Dockerfile
- For missing compose.production.yml → generate it (SKILL.md Step 4.4)
...
1. **Fill missing infrastructure choices** (same precedence as SKILL.md Step 1.3: task body → code detection → defaults; interactive sessions may ask) for any services not yet in compose:
