...
Before executing any phase (inline, or as the prompt for the corresponding `loop-*` subagent), set this context:
...
Subagent: `loop-planner` (subagents mode) or inline.
...
Subagent: `loop-producer` (subagents mode) or inline.
...
Runs in parallel with PRODUCE via the Agent tool (subagents: `loop-test-prep`, `loop-perf-prep`, `loop-invariant-prep` — pick those matching the active rule categories) or inline after PRODUCE in sequential fallback. Does not need the artifact — works from rules and task prompt.
  ...
  "checks": [
    {
      ...
      "type": "executable",
      "command": "test -s .hermes-dev/loop/<task-alias>/artifact.md",
...
Subagent: `loop-evaluator` (subagents mode) or inline. Uses artifact from PRODUCE and checks from PREPARE. Spawns parallel subagents for independent check groups.
...
- run prepared executable checks via parallel subagents with Bash
- run prepared content checks via parallel subagents with Read/Grep
...
Subagent: `loop-critic` (subagents mode) or inline.
...
Subagent: `loop-refiner` (subagents mode) or inline.
