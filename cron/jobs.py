...
# profile's jobs under that same HERMES_HOME. We deliberately anchor on
...
# into one shared `jobs.json` and leak config/credentials/skills across
# profiles (the security boundary #4707 was filed for).
#
# Unlike upstream's import-time constants, these resolve LAZILY via resolver
# functions so a single long-lived gateway that switches the active profile per
# routed turn always picks up the *current* profile home (our profile-isolation
# rework — commit "fix(cron): restore B1/B2 using clean resolver functions").
# The module-level attributes default to None and may be overridden by
# tests/web_server; when None the resolver falls back to get_hermes_home().
HERMES_DIR = None
CRON_DIR = None
JOBS_FILE = None
