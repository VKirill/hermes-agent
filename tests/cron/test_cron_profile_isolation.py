...
def test_cron_storage_anchors_at_profile_home(tmp_path, monkeypatch):
    ...
    # cron/jobs.py resolves its paths LAZILY via _hermes_dir()/_jobs_file()
    # (our profile-isolation rework — the module constants default to None and
    # are resolved from get_hermes_home() at call time), so the store anchors
    # at <profile>/cron for whichever profile is active at lookup time.
