...
def _safe_getcwd() -> str:
    ...
    try:
        ...
        return get_terminal_cwd() or os.path.expanduser("~")
...
def _get_env_config() -> Dict[str, Any]:
    ...
    # Read the terminal cwd but sanity-check it for container backends.
    # If Docker cwd passthrough is explicitly enabled, remap the host path to
    # /workspace and track the original host path separately. Otherwise keep the
    # normal sandbox behavior and discard host paths.
    from hermes_constants import get_terminal_cwd
    cwd = get_terminal_cwd() or default_cwd
    if cwd and not _is_ssh_remote_tilde_cwd(env_type, cwd):
        cwd = os.path.expanduser(cwd)
    host_cwd = None
    if env_type == "docker" and mount_docker_cwd:
        docker_cwd_source = get_terminal_cwd() or _safe_getcwd()
...
if __name__ == "__main__":
    ...
    from hermes_constants import get_terminal_cwd
    print(f"  TERMINAL_CWD: {get_terminal_cwd() or _safe_getcwd()}")
    from hermes_constants import display_hermes_home as _dhh
