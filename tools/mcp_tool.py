...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        # Profile-aware lookup (our MCP isolation) with breaker keyed on the
        # active fingerprint; use upstream's flow where only a MISSING server
        # short-circuits — a present-but-sessionless server falls through to
        # the reconnecting-probe logic below instead of being cut off here.
    ...
    with _lock:
        ...
        for k, v in servers.items():
            ...
            if fp not in _servers:
                new_servers[fp] = (k, v)
        # #50170 stale-cache wake is not adapted to fingerprint-keyed _servers;
        # parked servers recover on their next self-probe (pre-#50170 behavior).
        stale_cached = []
