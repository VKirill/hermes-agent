...
def _dispatch_nonstreaming_api_request(agent, api_kwargs: dict, *, make_client):
    ...
    if agent.api_mode == "anthropic_messages":
        # Claude Agent SDK mode owns its transport (the `claude` CLI) and has
        # no Anthropic SDK client. Preserve that local backend before applying
        # the request-local client ownership contract used by direct Anthropic.
        if getattr(agent, "_claude_agent_sdk_mode", None):
    ...
    while t.is_alive():
        ...
        if _stale_elapsed > _stale_timeout:
            ...
            try:
                ...
                # Claude Agent SDK mode registers no request client, so this is
                # intentionally a no-op there while its heartbeat timeout logic
                # below still reports and terminates the stalled turn.
