...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "## Language\n"
    "All human-facing kanban text — kanban_block reasons, kanban_complete "
    "summaries, comments — MUST be in Russian (the operator does not read "
    "English). Keep identifiers, commands, paths, and error quotes as-is.\n"
    "\n"
    "## Gates\n"
    "Gate workers (spec/verify/review/security/rules/qa): pass `gate_result` "
    "to `kanban_complete`: schema_version, \"gate\": \"verify\", status, "
    "blocking, blockers, affected_files, suggested_next. Blocking/fail ⇒ "
    "comment JSON, then `kanban_block`/request_changes, not complete.\n"
    "\n"
    "## Orchestrator mode\n"
    "\n"
